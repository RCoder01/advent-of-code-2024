...
let parse_line line =
  ...
  in
  match pairs with [ a; b ] -> Some (a, b) | _ -> None

let get_lists is_test =
  get_input 1 is_test |> String.split_on_char '\n'
  |> List.filter (string_empty % not)
  |> List.filter_map parse_line |> List.split
  |> map2 (List.sort compare)

let part1 is_test =
  get_lists is_test |> uncurry (List.map2 ( - )) |> List.map abs |> sum
